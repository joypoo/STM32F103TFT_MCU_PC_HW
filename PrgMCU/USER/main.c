...
//							 2024-08-04，优化串口IRQ数据接收和处理
//	存在BUG描述: 最后一包图像数据由于4k对齐占据了一个扇区，有点浪费，程序中舍去保存和现实（图像上会出现半行黑线），从节省空间考虑可以放在第一包数据里
//  功能描述   : 
//              STM32F103 TFT开发板系列 
//              STM32F103C8T6
...
void ScreenMain(void); //绘制主屏幕
void FlashRW(void); //Flash W25Q32读写测试
void ScreenMainItem(void); //绘制主屏幕图标选择框

int Pic2Flash(int nNo, int nPkgNo); //图片数据写入Flash（包括编号、头信息，每次写100dot像素的数据）
void ShowPicLine(int nNo, int nPkgNo); //显示一行图像数据
void ShowPicFlash(int nNo); //显示指定编号的FLASH上的图片到TFT上
void ShowPicComm(int nNo, int nPkgNo, int nStartX, int nStartY); //显示指定编号的串口传过来上的图片到TFT上
...
extern uint8_t Refinish1_Flag; //串口1接收完成标志位

int main()
{
	int nNo, nPkgNo;
	bool bRetWriteFlash = 1; //写Flash的返回值
	...
	Timer_init(19, 7199); //10Khz的计数频率，计数到20为2ms,用于键盘检测
	...
	LED = 0;
	PicShowFlag[63] = 1; //W25Q保存的图片标记（图像分布页面）
	PicShowFlag[74] = 1; 
	PicShowFlag[85] = 1; 
	PicShowFlag[99] = 1;
	...
	while (1)
	{
		if(Refinish1_Flag == 1) { //串口传过来的图像保存到W25Qxx Flash及在TFT上显示
			Refinish1_Flag = 0;
			Parse_data();
			UsartdisData();
			...
			if (usart1.RxBuff[0] == 'Z' && usart1.RxBuff[1] == 'X' && usart1.RxBuff[2] == 'I' && usart1.RxBuff[3] == '_' & \
				...
				printf("<-- 图像头：[0x%08X], No.:%03d, PkgNo:%d, Width:%d, High:%d\r\n", 
				...
				printf("<-- 图像数据区：[0x%08X], No.:%03d, PkgNo:%d\r\n", (usart1.RxBuff[9] - 1) * 200 + 8, nNo, nPkgNo);
		...
		if (bGetBMP280Flag == 1) { //大气压、温度传感器数据获取及显示
			bGetBMP280Flag = 0;
			Press = bmp280_press * 1000;
			Temp = bmp280_temp * 1000;
			High = high * 1000;
			...
			case LONG_KEY:
			  ...
			  printf("KEY: HOLD CLICK.\r\n");
				...
				if (nCountSingleClick == 1 && bScreenMain == 1) { //BMP280 info
					...
					} else if (nCountHoldClick == 2) { //显示
						LCD_ShowString(4, 0, (const u8*)"Then, My Upload Pic", LBBLUE, WHITE, 16, 1);
						LCD_ShowString(9, 0, (const u8*)"Then, My Upload Pic", LBBLUE, WHITE, 16, 1);//叠加加粗
...
void ScreenMain()
{
	...
	LCD_ShowString(5, 0, (const u8*)"ZoomeXplore.cn", RED, WHITE, 16, 0); //LCD_ShowChinese(95+24, 0, "探索", RED, WHITE, 16, 0);
...
{
	LCD_ShowString(20 + nCountSingleClick * 40 + 3, 25 + 33, (const u8*)"^", RED, WHITE, 16, 1);//gImage_x下指示
...
int Pic2Flash(int nNo, int nPkgNo)
{
	...
	if (nPkgNo == 0) {
		...
		//SPI_FLASH_SectorErase(nAdrStart + 8 * 4096);//最后一行图像占据了一个扇区，有点浪费，
																								//可以考虑把文件头数据另存到一个单独扇区上，
																								//类似MBR，文件分布、损坏的扇区分布都保存在一起，并做备份保存
	}
	//将发送缓冲区的数据写到flash中
